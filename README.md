...
1. **Pre-trained ML Models (`data/processed/*.pkl`)**: 
   * Not shipped in this submission -- generated on demand. Serialized Python objects containing trained models (Random Forest, Gradient Boosting, XGBoost). Running `python src/training_pipeline.py` or any of the benchmark scripts saves them here, so the GA can load them and immediately predict facility transportation costs in microseconds.
