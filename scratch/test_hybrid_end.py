...
def run_ga_uflp_hybrid_end(dataset, pop_size=120, n_gen=100, cx_pb=0.8, mut_pb=0.3, random_seed=42):
    ...
    # Initialize with normal greedy seeds (no initial local search to preserve diversity)
...
def main():
    ...
    t0 = time.time()
    for r in range(30):
        cost = run_ga_uflp_hybrid_end(ds_b, random_seed=42+r)
        costs_b.append(cost)
    ...
    t0 = time.time()
    for r in range(30):
        cost = run_ga_uflp_hybrid_end(ds_c, random_seed=42+r)
        costs_c.append(cost)
    t_elapsed = time.time() - t0
