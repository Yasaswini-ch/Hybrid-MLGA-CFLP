...
def run_ga_uflp_hybrid(dataset, pop_size=120, n_gen=100, cx_pb=0.8, mut_pb=0.3, random_seed=42):
...
def main():
    ...
    t0 = time.time()
    for r in range(30):
        cost = run_ga_uflp_hybrid(ds_b, random_seed=42+r)
        costs_b.append(cost)
    ...
    t0 = time.time()
    for r in range(30):
        cost = run_ga_uflp_hybrid(ds_c, random_seed=42+r)
        costs_c.append(cost)
    t_elapsed = time.time() - t0
