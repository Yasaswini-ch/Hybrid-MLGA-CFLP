...
def run_ga_uflp_hybrid_init(dataset, pop_size=120, n_gen=100, cx_pb=0.8, mut_pb=0.3, random_seed=42):
    ...
    # Initialize with locally optimized greedy seeds
    pop[0] = creator.Individual(local_search_uflp(greedy_uflp_seed(dataset.fixed_costs, dataset.transport_costs, randomness=0.0), dataset.fixed_costs, dataset.transport_costs)[0])
    ...
    # Also run local search at the end
...
def main():
    ...
    t0 = time.time()
    for r in range(30):
        cost = run_ga_uflp_hybrid_init(ds_b, random_seed=42+r)
        costs_b.append(cost)
    ...
    t0 = time.time()
    for r in range(30):
        cost = run_ga_uflp_hybrid_init(ds_c, random_seed=42+r)
        costs_c.append(cost)
    t_elapsed = time.time() - t0
