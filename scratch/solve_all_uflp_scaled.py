...
for name in ["capa", "capb", "capc"]:
    ...
    prob += pulp.lpSum(d.fixed_costs[i] * y[i] for i in range(d.num_facilities)) + pulp.lpSum((d.transport_costs[j, i] / d.demands[j]) * x[j, i] for j in range(d.num_customers) for i in range(d.num_facilities))
    ...
    trans_sum = sum((d.transport_costs[j, i] / d.demands[j]) * pulp.value(x[j, i]) for j in range(d.num_customers) for i in range(d.num_facilities))
    
    print(f"{name}: open={num_open}, fixed_sum={fixed_sum}, trans_sum={trans_sum:.2f}, objective={pulp.value(prob.objective):.2f}")
