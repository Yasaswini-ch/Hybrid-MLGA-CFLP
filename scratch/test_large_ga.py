...
def run_ga_uflp_large(dataset, pop_size=200, n_gen=300, cx_pb=0.8, mut_pb=0.3, random_seed=42):
...
def main():
    ...
    costs_b = []
    print("Benchmarking capb with larger GA (15 runs)...")
    eval_cache.clear()
    t0 = time.time()
    for r in range(15):
        cost = run_ga_uflp_large(ds_b, random_seed=42+r)
        costs_b.append(cost)
    t_elapsed = time.time() - t0
    print(f"Time: {t_elapsed:.2f}s | capb Min cost found: {min(costs_b)} | Target: 12979071.58")
